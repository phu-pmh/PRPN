...
from model_nli import SNLIClassifier
class PRPN(nn.Module):
    ...
    def __init__(self, ntoken, ninp, nhid, nlayers,
                 ...
                 tie_weights=False, hard=False, res=1, num_classes=3, clf_hidden_dim=300, clf_num_layers=2, use_batchnorm=False):
        ...
        self.classifier = SNLIClassifier(
            num_classes=num_classes, input_dim=nhid,
            hidden_dim=clf_hidden_dim, num_layers=clf_num_layers,
            use_batchnorm=use_batchnorm, dropout_prob=dropout)
    ...
    def forward(self, pre, hyp, batch_size):
        pre_hidden = self.init_hidden(batch_size)
        hyp_hidden = self.init_hidden(batch_size)

        pre_h, _ = self.sent_encoder(pre, pre_hidden)
        hyp_h, _ = self.sent_encoder(hyp, hyp_hidden)
    ...
    def sent_encoder(self, input, hidden_states):
        ...
        reader_state, parser_state = hidden_states  # memory_h: bsz, nslots, nhid
        ...
        for i in range(input.size(0)):
            ...
            attentions.append(memory_gate_next[i])
        ...
        #max pooling of reader's hidden states
        output_h = torch.max(output_h, 0)[0]
        ...
        return output_h, (reader_state, parser_state)

    def init_hidden(self, bsz):
        return [self.reader[i].init_hidden(bsz)
                for i in range(self.nlayers)], \
               self.parser.init_hidden(bsz) # , \
               #None #self.predictor.init_hidden(bsz)
